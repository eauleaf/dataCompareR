...
locateMismatches <- function(DFA, DFB, keys=NULL, maxMismatches=NA){
  ...
  colTypeDiff <- sapply(select(DFA,.dots=colCompare), collapseClasses) == sapply(select(DFB,.dots=colCompare), collapseClasses)
  ...
  if(length(cols2Compare)>0) {
    ...
    if(length(cols2Diff) > 0) {
      ...
      # Get these cols once
      subsetA <- select(DFA,.dots = cols2Diff)
      subsetB <- select(DFB,.dots = cols2Diff)
  ...
  if(nrow(mismatchOut) == 0) {
    ...
    mismatchOut <- mismatchOut %>% select(.dots=colNames)
  }
